...
void Muxer::openFile() {
    ...
    if (!(fmt_ctx_->flags & AVFMT_NOFILE)) {
        if (avio_open2(&fmt_ctx_->pb, filename_.c_str(), AVIO_FLAG_WRITE, nullptr, nullptr) < 0) {
...
void Muxer::closeFile() {
    ...
    if (av_write_trailer(fmt_ctx_.get()) < 0) throw std::runtime_error("Muxer: Failed to write file trailer");
...
void Muxer::writePacket(av::PacketUPtr packet, av::DataType packet_type) const {
    ...
    if (av_interleaved_write_frame(fmt_ctx_.get(), packet.get()))
        throw std::runtime_error("Muxer: Failed to write packet");
}

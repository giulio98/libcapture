...
static void throwLogicError(const std::string &msg) { throw std::logic_error("Converter: " + msg); }
...
Converter::Converter(const AVCodecContext *dec_ctx, const AVCodecContext *enc_ctx, const AVRational in_time_base,
    ...
    if (!dec_ctx) throwLogicError("dec_ctx is NULL");
    if (!enc_ctx) throwLogicError("enc_ctx is NULL");
    ...
    if (dec_ctx->codec_type != enc_ctx->codec_type) {
        throwLogicError("type mismatch between received decoder and encoder");
        ...
        if (offset_x || offset_y) throwLogicError("video offset specified specified for audio converter constructor");
        ...
        throwLogicError("unknown media type received in constructor");
    }

    filter_graph_ = av::FilterGraphUPtr(avfilter_graph_alloc());
    if (!filter_graph_) throwLogicError("failed to allocate filter graph");

    { /* buffer src set-up*/
        const AVFilter *filter = avfilter_get_by_name(src_filter_name.c_str());
        if (!filter) throwLogicError("failed to find src filter definition");
        if (avfilter_graph_create_filter(&buffersrc_ctx_, filter, "in", src_args.c_str(), nullptr,
                                         filter_graph_.get()) < 0)
            throwLogicError("failed to create src filter");
    }

    { /* buffer sink set-up*/
        const AVFilter *filter = avfilter_get_by_name(sink_filter_name.c_str());
        if (!filter) throwLogicError("failed to find sink filter definition");
...

    {
        ...
        if (!outputs) throwLogicError("failed to allocate filter outputs");
        ...
        if (!inputs) throwLogicError("failed to allocate filter inputs");
        ...
        if (ret < 0) throwLogicError("failed to parse pointers");
    }

    if (avfilter_graph_config(filter_graph_.get(), nullptr) < 0)
        throwLogicError("failed to configure the filter graph");
}
...
void Converter::sendFrame(const av::FrameUPtr frame) {
    if (!buffersrc_ctx_) throwLogicError("buffersrc is not allocated");
    if (!frame) throwLogicError("sent frame is not allocated");
    if (av_buffersrc_add_frame(buffersrc_ctx_, frame.get())) throwLogicError("failed to write frame to filter");
}

av::FrameUPtr Converter::getFrame() {
    if (!buffersink_ctx_) throwLogicError("buffersink is not allocated");
    ...
    if (ret < 0) throwLogicError("failed to receive frame from filter");

...
static void throwError(const std::string &msg) { throw std::runtime_error("Demuxer: " + msg); }
...
Demuxer::Demuxer(const std::string &fmt_name, std::string device_name, std::map<std::string, std::string> options)
    ...
    if (!fmt_) throwError("cannot find input format");
}
...
void Demuxer::openInput() {
    if (fmt_ctx_) throwError("failed to open input device (input is already open)");
    if (!fmt_) throwError("failed to open input device (missing input format)");

    {
        ...
        if (ret) throwError("cannot open input format");
#if VERBOSE
    ...
    if (avformat_find_stream_info(fmt_ctx_.get(), nullptr) < 0) throwError("Failed to find stream info");
...
void Demuxer::closeInput() {
    if (!fmt_ctx_) throwError("failed to close input (input is not open)");
    fmt_ctx_.reset();
...
void Demuxer::flush() {
    if (!fmt_ctx_) throwError("failed to flush (input is not open)");
    if (fmt_ctx_->pb) avio_flush(fmt_ctx_->pb);
    if (avformat_flush(fmt_ctx_.get()) < 0) throwError("failed to flush internal data");
}
...
const AVCodecParameters *Demuxer::getStreamParams(av::MediaType stream_type) const {
    if (!fmt_ctx_) throwError("failed to acess stream (input is not open)");
    if (!av::validMediaType(stream_type)) throwError("invalid stream_type received");
    if (!streams_[stream_type]) throwError("specified stream not present");
...
std::pair<av::PacketUPtr, av::MediaType> Demuxer::readPacket() {
    if (!fmt_ctx_) throwError("failed to read packet (input is not open)");

    if (!packet_) {
        packet_ = av::PacketUPtr(av_packet_alloc());
        if (!packet_) throwError("failed to allocate packet");
    }
    ...
    if (ret < 0) throwError("failed to read a packet");
    ...
    if (packet_type == av::MediaType::None) throwError("unknown packet stream index");

    return std::make_pair(std::move(packet_), packet_type);
}

void Demuxer::printInfo(int index) const {
    if (!fmt_ctx_) throwError("failed to print info (input is not open)");

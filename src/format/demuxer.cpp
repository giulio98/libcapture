...
void Demuxer::openInput() {
    if (fmt_ctx_) throwRuntimeError("failed to open input device (input is already open)");
    if (!fmt_) throwRuntimeError("failed to open input device (missing input format)");

    {
        ...
        if (ret) throwRuntimeError("cannot open input format");
#if VERBOSE
    ...
    if (avformat_find_stream_info(fmt_ctx_.get(), nullptr) < 0) throwRuntimeError("Failed to find stream info");
...
void Demuxer::closeInput() {
    if (!fmt_ctx_) throwRuntimeError("failed to close input (input is not open)");
    fmt_ctx_.reset();
...
void Demuxer::flush() {
    if (!fmt_ctx_) throwRuntimeError("failed to flush (input is not open)");
    if (fmt_ctx_->pb) avio_flush(fmt_ctx_->pb);
    if (avformat_flush(fmt_ctx_.get()) < 0) throwRuntimeError("failed to flush internal data");
}
...
std::pair<av::PacketUPtr, av::MediaType> Demuxer::readPacket() {
    if (!fmt_ctx_) throwRuntimeError("failed to read packet (input is not open)");
    ...
    if (packet_type == av::MediaType::None) throwRuntimeError("unknown packet stream index");

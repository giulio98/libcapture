...
int open_media(const char *in_filename, AVFormatContext **avfc) {
    *avfc = avformat_alloc_context();
    if (!*avfc) {
        cerr << "failed to alloc memory for format";
        return -1;
    }

    if (avformat_open_input(avfc, in_filename, NULL, NULL) != 0) {
        cerr << "failed to open input file %s", in_filename;
        return -1;
    }

    if (avformat_find_stream_info(*avfc, NULL) < 0) {
        cerr << "failed to get stream info";
        return -1;
    }
    return 0;
}
...
int prepare_decoder(StreamingContext *sc) {
    for (int i = 0; i < sc->avfc->nb_streams; i++) {
        if (sc->avfc->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            cerr << "skipping streams other than audio and video";
        }
    }
...
int prepare_video_encoder(StreamingContext *sc, AVCodecContext *decoder_ctx, AVRational input_framerate,
    ...
    if (!sc->video_avc) {
        cerr << "could not find the proper codec";
        return -1;
    }

    sc->video_avcc = avcodec_alloc_context3(sc->video_avc);
    if (!sc->video_avcc) {
        cerr << "could not allocated memory for codec context";
        return -1;
    }
...
int remux(AVPacket **pkt, AVFormatContext **avfc, AVRational decoder_tb, AVRational encoder_tb) {
    ...
    if (av_interleaved_write_frame(*avfc, *pkt) < 0) {
        cerr << "error while copying stream packet";
        return -1;
    }
    return 0;
}
...
int encode_video(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame) {
    ...
    if (!output_packet) {
        cerr << "could not allocate memory for output packet";
        return -1;
    }
    ...
    while (response >= 0) {
        ...
        if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
            ...
            cerr << "Error while receiving packet from encoder: " << av_err2str(response);
            return -1;
        }
        ...
        if (response != 0) {
            cerr << "Error " << response << " while receiving packet from decoder: " << av_err2str(response);
            return -1;
        }
    }
...
int encode_audio(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame) {
    ...
    while (response >= 0) {
        ...
        response = av_interleaved_write_frame(encoder->avfc, output_packet);
        if (response != 0) {
            cerr << "Error %d while receiving packet from decoder: %s", response, av_err2str(response);
            return -1;
        }
    }
    av_packet_unref(output_packet);
...
int transcode_audio(StreamingContext *decoder, StreamingContext *encoder, AVPacket *input_packet,
    ...
    if (response < 0) {
        cerr << "Error while sending packet to decoder: %s", av_err2str(response);
        return response;
    }
    ...
    while (response >= 0) {
        ...
        if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
            ...
            cerr << "Error while receiving frame from decoder: " << av_err2str(response);
            return response;
        }
...
int transcode_video(StreamingContext *decoder, StreamingContext *encoder, AVPacket *input_packet,
    ...
    if (response < 0) {
        cerr << "Error while sending packet to decoder: " << av_err2str(response);
        return response;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (!encoder->avfc) {
        cerr << "could not allocate memory for output format";
        return -1;
    }
    ...
    if (encoder->avfc->oformat->flags & AVFMT_GLOBALHEADER) encoder->avfc->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
    ...
    if (avformat_write_header(encoder->avfc, &muxer_opts) < 0) {
        cerr << "an error occurred when opening output file";
        return -1;
    }

    AVFrame *input_frame = av_frame_alloc();
    if (!input_frame) {
        cerr << "failed to allocated memory for AVFrame";
        return -1;
    }

    AVPacket *input_packet = av_packet_alloc();
    if (!input_packet) {
        cerr << "failed to allocated memory for AVPacket";
        return -1;
    }
    ...
    while (av_read_frame(decoder->avfc, input_packet) >= 0) {
        if (decoder->avfc->streams[input_packet->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            cerr << "ignoring all non video or audio packets";
        }
    }
